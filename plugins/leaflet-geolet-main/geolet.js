...
(function () {
	...
	var _defaultMarker = function (controlOptions) {
		...
		return L.marker(null, {
			icon: L.divIcon({
				html: '<div class="geolet-active-outer"><div class="geolet-active-inner"></div></div>',
				iconAnchor: [10, 10],
				className: ''
			}),
	...
	var _defaultMarkerBackup = function (controlOptions) {
		//Generate a default marker
		return L.marker(null, {
			icon: L.divIcon({
				html: '<svg width="20" height="20" viewport="0 0 20 20"><circle cx="10" cy="10" r="10" style="fill:red"/><circle cx="10" cy="10" r="7" style="fill:none;stroke:white;stroke-width:3px"/><circle cx="10" cy="10" r="4" style="fill:red"/></svg>',
				iconAnchor: [10, 10],
				className: ''
			}),
			attribution: '<a href="https://github.com/rhlt/leaflet-geolet" target="_blank">Geolet</a>',
	...
	L.Geolet = L.Control.extend({
		...
		options: {
			...
			style: {outer:'geolet-control-outer',inner:'geolet-control-inner'},
			activeStyle: { outer:'geolet-active-outer',inner:'geolet-active-inner' },
			html: '<div class="geolet-control-holder"><div class="geolet-control-outer"><div class="geolet-control-inner"></div></div></div>',
		...
		styleAnchor: function (active) {
			...
			if (active && this.options.activeStyle){
				//_merge(this._a.style, this.options.activeStyle);options.activeStyle.outer
				//console.log(this.options.activeStyle.outer);
				this._a.getElementsByTagName("div")[1].setAttribute("class", this.options.activeStyle.outer);
				this._a.getElementsByTagName("div")[2].setAttribute("class", this.options.activeStyle.inner);
			}
				
			if (!active && this.options.style){
				//_merge(this._a.style, this.options.style);
				if(document.readyState === "complete") {
					this._a.getElementsByTagName("div")[1].setAttribute("class", this.options.style.outer);
					this._a.getElementsByTagName("div")[2].setAttribute("class", this.options.style.inner);
				}
			}
		},
		...
		onAdd: function (map) {
			...
			if (!L.Geolet.browserSupport) {
				...
				el.classList.add("geolet-dsp-none");
				return el;
			}
			...
			this._a.className = 'geolet-control-holder';
